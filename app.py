# GitHub Copilot Chat Assistant - refactored Health Predictor app
...
def load_pickle_resource(model_name: str):
    """Try multiple locations for the model and load with pickle (cached)."""
    possible_paths = [
...
def predict_with_model(model, features: np.ndarray, scaler=None) -> (float, int):
    """Return (probability_of_positive, predicted_label)"""
    if scaler is not None:
        try:
            if hasattr(scaler, "transform"):
                features = scaler.transform(features)
        except Exception:
            # ignore scaler issues; continue with raw features
            pass
...
if selection == "Home":
    ...
    with col1:
        st.write("This demo provides quick, friendly health risk predictions using your inputs. "
                 "The UI uses cards, Lottie animations, and interactive gauges for nicer feedback.")
        st.write("Tips:")
